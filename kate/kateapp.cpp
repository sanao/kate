...
void KateApp::restoreKate()
{
    ...
    sessionManager()->activateSessionById(lastSession, false, false);
...
bool KateApp::startupKate()
{
    ...
    if (m_args.isSet(QStringLiteral("start"))) {
        const bool success = sessionManager()->activateSessionByName(m_args.value(QStringLiteral("start")));
        if (!success) {
            // FIXME: do something else when no such session is found?
            return false;
        }
